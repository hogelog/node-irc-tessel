...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    if (self.opt.secure) {
        throw "Unsupported option: secure";
    }
    ...
    self.conn.addListener("connect", function () {
        ...
        } else if ( self.opt.password !==  false ) {
...
Client.prototype.say = function(target, text) { // {{{
    ...
    if (typeof text !== 'undefined') {
        text.toString().split(/\r?\n/).filter(function(line) {
            ...
            var messagePart;
            var messageSplit = self.opt.messageSplit;
            while ((messagePart = line.substring(0, messageSplit)).length > 0) {
                self.send('PRIVMSG', target, messagePart);
                self.emit('selfMessage', target, messagePart);
                line = line.substring(messageSplit, line.length);
            }
        });
    }
} // }}}
